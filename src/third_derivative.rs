...
pub fn third_derivative_geometric(
    ...
    let hal = crate::halogen::halogen_third_derivative(system)?;
    let n = rep.len();
...
///
/// **State (corrected 2026-07-01 by decomposition at a non-EQ water geometry).** The earlier claim
/// that the 2n+1 interchange omits a second-order response-Hessian derivative `∂_s R_rc` was WRONG:
/// the closed-form `D_c(hessian_response)` matches `FD(hessian_response)` to ~7e-7 even at non-EQ,
/// and every geometric frozen block's 3rd derivative matches FD to ~1e-10 — the 2n+1 response route
/// is correct and needs only first-order responses (as Wigner's 2n+1 rule requires). The O(100%)
/// non-EQ error was TWO bugs, neither in the response theory:
///   1. **[FIXED] dispersion gating** — [`third_derivative_frozen_complete`] added the D3 3rd
///      derivative unconditionally (nonzero even for `reference_path = None`), so with dispersion
///      DISABLED the analytic path carried a spurious D3 term absent from the seminumerical ground
///      truth. D3's 3rd derivative blows up at compressed geometries → the O(100%). Now gated on
///      `include_dispersion && enable_dispersion` (matching the Hessian). Dominant contribution.
///   2. **[remaining, smaller, pre-existing]** a ~0.2% residual in the frozen-density-path `l_abx`
///      ([`frozen_hessian_density_path`]) — localized to the **Pulay V-channel** bilinear-`P·V`
///      approximation (the largest density-path, ~5e-2, carries a ~1% error). A first-order-response
///      contraction (consistent with 2n+1). Not yet fixed; gated by
///      `third_derivative_nonEq_matches_seminumerical` at the current residual.
/// After fix (1), non-EQ analytic vs seminumerical is ~6e-4 (was ~1e-2+ with the spurious D3). **For
/// production accuracy use the seminumerical path** [`third_derivative_seminumerical_dense`]/`_block`/`_vector` (~1e-4, validated).
/// Every component is independently FD-gated (`f_bc_full_matches_fd`, `d_c_fock_mo_derivative_matches_fd`,
/// `d_c_rhs_{nonmetric,metric}_matches_fd`, `d_c_operator_action_matches_fd`, `d_c_orbital_bundle_derivative_matches_fd`,
/// the Group-A `d_c_groupa_*`, `d_c_orbital_adjoint_total_matches_fd`, `d_c_static_sector_matches_fd`,
/// `closed_form_response_matches_hessian_response_fd`). The seminumerical path
/// [`third_derivative_seminumerical_dense`]/`_block`/`_vector` (~1e-4, two analytic-Hessian evals) remains the
/// cheap alternative. Returned dense slabs `slab[c][(a,b)] = T_abc`. For best accuracy use tight SCF in
/// `electronic_options` (`energy_tolerance 1e-11`, `charge_tolerance 1e-9`).
///
...
fn frozen_hessian_density_path(
    system: &PeriodicSystem,
    params: &Gfn1Parameters,
    electronic: &ElectronicResult,
    coordination_cutoff: f64,
...
#[allow(clippy::too_many_arguments)]
fn closed_form_response_hessian_derivative(
    system: &PeriodicSystem,
    params: &Gfn1Parameters,
    electronic: &ElectronicResult,
    ...
    let resp: Vec<M> = (0..ndof)
        ...
        .map(|c| -> Result<M> {
            ...
            for b in 0..ndof {
                ...
                let d_sp_o: Vec<f64> = {
                    let kdq = kvec(&d_dq_o);
                    (0..nshell).map(|s| dk_dqo[(s, c)] + kdq[s]).collect()
                };
                let d_rf_o = {
                ...
                let d_sp_s: Vec<f64> = {
                    let kdq = kvec(&d_dq_s);
                    (0..nshell).map(|s| dk_dqs[(s, c)] + kdq[s]).collect()
                };
                let d_rf_s = {
                ...
                for (idx, &(i, a)) in space.pairs.iter().enumerate() {
                    let drhs0 = -d_f_tilde[(i, a)]
                        + eps_c[c][i] * s_tilde_b[(i, a)]
                        + eps[i] * d_s_tilde[(i, a)];
...
mod tests {
    ...
    fn third_derivative_geometric_matches_hessian_finite_difference() {
        ...
        let hess = |sys: &PeriodicSystem| -> Matrix {
            let mut m = crate::repulsion::repulsion_energy_gradient_hessian(sys, &params)
                .unwrap()
                .hessian;
            let hal = crate::halogen::halogen_energy_gradient_hessian(sys)
                .unwrap()
                .hessian;
            for r in 0..ndof {
                for c in 0..ndof {
    ...
    // The experimental analytic 2n+1 third derivative vs the central FD of the FULL analytic Hessian.
    // The analytic response is RESEARCH-LEVEL (~few percent of the response; the exact symmetric 2n+1
    // also needs the response-Hessian derivative ∂_s R_rc, only reachable in the orbital-rotation
    // parametrization). So this gate asserts the response blocks *substantially improve* on the pure
    // frozen approximation -- it does NOT claim machine precision. The validated, production-accuracy
    // path is `third_derivative_seminumerical_*` (a separate test gates it to ~1e-4).
    #[test]
    fn third_derivative_analytic_improves_substantially_over_frozen() {
        ...
        // response-free geometric block).
        // NOTE: the residual ~4.8e-3 is NOT SCF-FD floor (tight SCF leaves it unchanged) — it is the
        // `frozen_hess_at` FD-scaffold error (D_c H_frozen), which the analytic L_abc/L_abx removes (Z6).
        ...
        // Matches FD(full Hessian) to ~7e-5 on water (limited by the h=1e-4 FD reference). Gate at 5e-4.
        assert!(
            err_analytic < 5.0e-4,
    ...
    fn load_params_diag() -> Option<Gfn1Parameters> {
        if let Ok(p) = std::env::var("GFN1_XTB_PARAM") {
            if let Ok(pp) = Gfn1Parameters::from_file(p) {
                return Some(pp);
            }
        }
    ...
    /// consistent with 2n+1). The analytic 3rd derivative now matches the seminumerical to ~7e-7 here, so
    /// the gate is tightened from 2e-3 to 1e-5 (still ~13× the residual, robust to seminumerical FD noise).
    #[test]
    fn third_derivative_nonEq_matches_seminumerical() {
        ...
        // With the Pulay CN-response term (fixed_density_pulay_cn_h0_response) the analytic path matches
        // the seminumerical to ~7e-7 at this non-EQ geometry. Gate at 1e-5 (13× margin, robust to the
        // seminumerical's own FD noise); the pre-fix bug was O(100%) of `refm`.
        assert!(
            err < 1.0e-5,
    ...
    fn diag_nonEq_third_derivative_decompose() {
        ...
        block_err("geo(rep+hal)", &geo3, &|s| {
            ...
            let hh = crate::halogen::halogen_energy_gradient_hessian(s).unwrap().hessian;
        ...
        let blocks_sum_ref = {
            ...
            let hh = crate::halogen::halogen_energy_gradient_hessian(&system).unwrap().hessian;
    ...
    fn bridge_frozen_reconstruction_equals_analytic_hessian() {
        ...
        let rep = crate::repulsion::repulsion_energy_gradient_hessian(&system, &params)
            .unwrap()
            .hessian;
        let hal = crate::halogen::halogen_energy_gradient_hessian(&system)
            .unwrap()
            .hessian;
        let scc = crate::hessian::fixed_shell_charge_scc_hessian(
            &system,
